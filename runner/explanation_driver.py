...
class ExplanationDriver(TA3Driver):
    ...
    def decide(self, itm_probe: ext.ITMProbe) -> ext.Action:
        ...
        if len(act.kdmas) > 0:
            ...
            for kdmaName, val in act.kdmas.items():
                ...
                self.estimated_kdma_counts[kdma] = (
                    self.estimated_kdma_counts.get(kdma, 0) + 1
                )
                self.estimated_kdmas[kdma] = self.estimated_kdmas.get(kdma, 0) + val
                kdmamin = min(
                    [
                        opt.kdmas[kdmaName]
                        for opt in itm_probe.options
                        if opt.kdmas is not None
                    ]
                )
                kdmamax = max(
                    [
                ...
                # breakpoint()
                self.estimated_min_kdmas[kdma] = (
                    self.estimated_min_kdmas.get(kdma, 0) + kdmamin
                )
                self.estimated_max_kdmas[kdma] = (
                    self.estimated_max_kdmas.get(kdma, 0) + kdmamax
                )
        return act
    ...
    def train(self, feedback: ta3.AlignmentResults, final: bool):
        ...
        for kdma, count in self.estimated_kdma_counts.items():
            print(
                "%s: Target: %3f Estimated: %3f Minimum: %3f Maximum: %3f"
                % (
                    kdma,
